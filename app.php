...
$query = $db->prepare("SELECT `Type_ID` FROM `type` WHERE `Type` = ?");
$query->execute([$type]);
...
$query = $db->prepare("SELECT `Genre_ID` FROM `genre` WHERE LOWER(`Genre`) IN ($questionmarks)");
...
$query = $db->prepare("SELECT * FROM `moviedb` WHERE `Type_ID` = ? AND `Genre_ID` IN ($questionmarks) ORDER BY RAND() LIMIT 5");
